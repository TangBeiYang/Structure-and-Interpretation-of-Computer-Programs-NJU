...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    result=typed_word
    for y in word_list:
        diff = diff_function(typed_word,y,limit)
        if diff<flag:
            flag=diff
            result=y
    return result if flag <= limit else typed_word
    # END PROBLEM 5
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(lm):
        ...
        yfy+=1
    result=yfy/l2
    current={'id': user_id, 'progress': result}
    upload(current)
    return result
    # END PROBLEM 8
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    yfy=get_all_times(game)
    tye=get_all_words(game)
    result=[]
