...
def scheme_eval(expr, env, tail=False):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        yfy=rest.map(lambda x:scheme_eval(x,env))
        return scheme_apply(f,yfy,env)
        # END PROBLEM 3
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        yfy=[]
        while args != nil:
            yfy.append(args.first)
            args=args.rest
        if procedure.expect_env:
            yfy.append(env)
        try:
            return procedure.py_func(*yfy)
        except TypeError:
        ...
        # BEGIN PROBLEM 11
        kid=env.make_child_frame(procedure.formals,args)
        return eval_all(procedure.body,kid)
        # END PROBLEM 11
    else:
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(res, Unevaluated):
            res = original_scheme_eval(res.expr, res.env,False)
        return res
        # END PROBLEM EC
