...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while temp.is_hive == False:
            if self.min_range<=rrange<=self.max_range:
                if temp.bees !=[]:
...
class HungryAnt(Ant):
    ...
    chew_duration=3
    def action(self,gamestate):
        if self.place.bees==[] or self.chew_countdown>0:
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.ant_contained==None:
            return
        annt=self.ant_contained
        annt.action(gamestate)
        # END PROBLEM 8
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        while temp!=None:
            annt=temp.ant
            if annt!=None:
                if annt.is_container==True and annt.ant_contained!=None:
                    annt.buff()
                    annt.ant_contained.buff()
                annt.buff()
            temp=temp.exit
        super().action(gamestate)
        # END PROBLEM 12
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while temp.is_hive == False:
            if temp.bees !=[]:
                ...
                for bee in beee:
                    result[bee]=dis
            if temp.ant is not None and dis>0:
                result[temp.ant]=dis    
            temp=temp.entrance
            dis+=1
        return result
    ...
    def calculate_damage(self, distance):
        # BEGIN PROBLEM Optional 2
        a=self.damage-0.25*distance-(self.insects_shot)*0.0625
        return max(0,a)
        # END PROBLEM Optional 2
